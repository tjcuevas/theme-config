...
const getCustomProperties = (styleSheet: StyleSheet): any[] => {
	return [...(styleSheet as any).cssRules].filter(rule => rule.style != null).reduce((properties, rule) => {
...
const ThemeValues = () => {
	...
	useEffect(() => {
		...
		const customProperties = styleSheets.reduce<any[]>((allProperties, styleSheet) => [...allProperties, ...getCustomProperties(styleSheet)], []);
...
const CustomPropertyInput = ({ name, initialValue }: ICustomPropertyInputProps) => {
		...
		<div>
			...
			<Input
				...
				onChange={(event) => {
					...
					if (rootElement) {
						(rootElement as any).style.setProperty(event.target.name, event.target.value);
					}
